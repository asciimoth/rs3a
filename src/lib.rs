...
impl fmt::Display for ParcingError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::UnknownColor(c) => {
                write!(f, "UnknownColor: {}", c)
            }
            Self::UnknownColorMod(s) => {
                write!(f, "UnknownColorMod: {}", s)
            }
            Self::InvalidWidth => {
                write!(f, "InvalidWidth")
            }
            Self::InvalidHeight => {
                write!(f, "InvalidHeight")
            }
            Self::ThereIsNoBody => {
                write!(f, "There is no body found")
            }
        }
    }
}
    ...
    pub fn from_string(s: String, h: Header) -> Result<Self, ParcingError> {
        ...
        'outer: while next {
            ...
            for y in 0..height {
                ...
                for x in 0..width {
                    let symbol_pos: usize =
                        (frm * width * datacols * height) + (y * width * datacols) + x;
                    if symbol_pos >= len {
                        next = false;
                        break;
                    }
                    ...
                    match h.color_mod {
                        ColorMod::None => {}
                        ColorMod::Fg => {
                            let fg_color_position = (frm * width * datacols * height)
                                + (y * width * datacols)
                                + width
                                + x;
                        ...
                        ColorMod::Bg => {
                            let bg_color_position = (frm * width * datacols * height)
                                + (y * width * datacols)
                                + width
                                + x;
        ...
        for (frm, frame) in self.frames.iter().enumerate() {
            for row in frame {
                ...
                for fragment in row {
                    ...
                    color1_col +=
                        &Self::generate_color_fragment(fragment.fg_color, fragment.text.len());
                    color2_col +=
                        &Self::generate_color_fragment(fragment.bg_color, fragment.text.len());
                }
                ret += &text_col;
                ret += &color1_col;
                ret += &color2_col;
...
impl TryFrom<String> for Header {
    ...
    fn try_from(s: String) -> Result<Self, Self::Error> {
        ...
        if !w_set {
            return Err(ParcingError::InvalidWidth);
        }
        if !h_set {
            return Err(ParcingError::InvalidHeight);
        }
        if !d_set {
